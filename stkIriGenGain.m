function [H, Ttol, Hsat, TStol] = stkIriGenGain(sub_bandNumber,pathStr)
    % 读取scenario
...
H = H * 10^4;
     ...
     % 星间可见矩阵
sat_allAccessIntervals = [];
hWait = waitbar(0,'Please Wait for Computing ISL  ╭(′▽`)╭(′▽`)╯');
for from = 0:1:satNum-1
   ...
   for to = 0:1:satNum-1
       ...
       eval(['toSat=sat',num2str(to),';']);
%        计算可见性
       IS_access = fromSat.GetAccessToObject(toSat);
       IS_access.ComputeAccess();
%        AS_access.get;
       IS_accessIntervals = IS_access.ComputedAccessIntervalTimes;
       if(IS_accessIntervals.Count~=0)
           computedIntervals = IS_accessIntervals.ToArray(0, -1); %时间区间
           temp=cell(IS_accessIntervals.Count,2);                 %存储缓存
           meanRange = cell(IS_accessIntervals.Count,1);
           
           for i=1:1:IS_accessIntervals.Count
               temp{i,1}=from+1;
               temp{i,2}=to+1;
               [str,sto] = IS_accessIntervals.GetInterval(i-1);
               aerDP = IS_access.DataProviders.Item('AER Data').Group.Item('VVLH CBF').Exec(str,sto,1);
               range = cell2mat(aerDP.DataSets.GetDataSetByName('Range').GetValues);
               meanRange{i,1} = mean(range);
           end
           sat_allAccessIntervals = [sat_allAccessIntervals;temp,computedIntervals,meanRange];
...
close(hWait)
% uav_allAccessIntervals
% 每行表示卫星和卫星之间的连接，第1、2列表示卫星from和卫星to；第3、4列表示链接维持区间；第5列表示卫星和无人机之间的平均距离
[access_row,access_col] = size(sat_allAccessIntervals);
ISL_Access_Cell = cell(access_row, access_col);
for row = 1:access_row
    ISL_Access_Cell{row,1} = sat_allAccessIntervals{row, 1};
    ISL_Access_Cell{row,2} = sat_allAccessIntervals{row, 2};
    buff_cell = sat_allAccessIntervals(row, 3);
    buff_str = buff_cell{1};
    buff_str = buff_str(1:end-4);
    buff_t = datetime(buff_str,'Locale','en_US','InputFormat',format2datetime);
    ISL_Access_Cell{row, 3} = buff_t;
    buff_cell = sat_allAccessIntervals(row, 4);
    buff_str = buff_cell{1};
    buff_str = buff_str(1:end-4);
    buff_t = datetime(buff_str,'Locale','en_US','InputFormat',format2datetime);
    ISL_Access_Cell{row, 4} = buff_t;
    ISL_Access_Cell{row, 5} = sat_allAccessIntervals(row, 5);
end
ISL_range_Ttol = [];
temp = cell(1,5);
% 第一列为UAV、第二列为卫星、第三列表示距离，第四列表示剩余时间Ttol，第5列表示信道增益
for row = 1:access_row
   if(ISL_Access_Cell{row, 3} < check_time_date)&&(ISL_Access_Cell{row, 4} > check_time_date)
       temp{1} = ISL_Access_Cell{row, 1};
       temp{2} = ISL_Access_Cell{row, 2};
       temp{3} = ISL_Access_Cell{row, 5}{1};
       buff = ISL_Access_Cell{row, 4} - check_time_date;
       temp{4} = seconds(buff);
%        temp{5} = genH(temp{3},6000);             %信道使用C波段6GHz频率
       temp{5} = genH(temp{3},27000);              %信道使用Ka波段17GHz频率
       ISL_range_Ttol = [ISL_range_Ttol; temp];
   end
end
sub_bandNumber = 1;
% 将上述计算结果构建为信道矩阵和Ttol矩阵返回
Hsat = zeros(satNum,satNum,sub_bandNumber);
TStol = zeros(satNum,satNum,sub_bandNumber);
[ISL_row,~] = size(ISL_range_Ttol);
for row = 1:ISL_row
    uavIndex = ISL_range_Ttol{row,1};
    satIndex = ISL_range_Ttol{row,2};
    Hsat(uavIndex,satIndex,:) = ISL_range_Ttol{row,5} * ones(1,sub_bandNumber);
